...
import { GetReferralUser, GetUserById, GetUserReferral, UpdateReferralUser } from "@/src/redux/service/AdminApi";
...
export default function Referrals() {
  ...
  const [referral, setReferral] = useState({ userId: "", referral_amount: 0, isActive: false });
  ...
  const fetchUser = async () => {
    ...
    if (selectedUser) {
      ...
      if (userReferral.status){
        ...
        setReferral({userId:userId,referral_amount:referral_amount,isActive:isActive})
      }
    }
  };
const reset = ()=>{
            ...
            <Grid container>
              <Grid item xs={12} md={6} className="pr-4">
                <label className="text-[#344054] text-sm font-medium">
                  Referral Incentive Beneficiary <span className="text-red-500">*</span>
                </label>
                <br />
                <select
                  className="w-full border-solid border py-2 border-opacity-20 pl-2 rounded-lg border-black-variant bg-[#FBFBFB] outline-none"
                  onChange={(e) => {setSelectedUser(e.target.value);setReferral({...referral,userId:e.target.value})}}
                  value={selectedUser}
                >
                  <option disabled>
              ...
              <Grid item xs={12} md={6} className="pr-4">
                ...
                <TextField
                  ...
                  size="small"
                  type="Number"
                  onChange={(e) => setReferral({ ...referral, referral_amount: e.target.value })}
              ...
              <Grid item xs={12} md={6} className="pr-4 mt-4">
                ...
                <select
                  ...
                  onChange={(e) => setReferral({ ...referral, isActive: e.target.value == 1 ? true : false })}
                >
              ...
              <Grid item xs={12}>
                <button onClick={handleUpdateReferralUser} className="bg-[#FFA500] text-white font-medium text-xs px-6 py-2 rounded-lg mt-5">
                  Add to system
                </button>
              </Grid>
            </Grid>
