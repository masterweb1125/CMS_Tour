...
const Client_ToursDirectoryItem = ({tour}:{tour:Tours}) => {
  return (
    <div className="card tour-card p-4 flex flex-col rounded-2xl gap-6  bg-white shadow-md">
      <div className="card-media relative cover bg-center h-56 w-full rounded-lg" style={{ backgroundImage: `url(${tour.coverPic})`}} >
        <div className="favorite-container bg-white absolute top-3 right-3 rounded-lg text-black">
          <IconButton><FavoriteBorderIcon fontSize='small' color='inherit' /></IconButton>
        </div>
      </div>

      <div className="card-content">
        <div className="title-price flex flex-row justify-between mb-5">
          <h2 className='text-2xl font-semibold text-black-variant leading-7'>{tour.name}</h2>
          <h4 className='text-primary text-2xl font-semibold leading-7'>${tour.price}</h4>
        </div>

        <div className="agent-details mb-5 flex flex-row items-center gap-2">
          <Image src={tour.agent?.avatarUrl || ''} width={20} height={20} alt={tour?.agent?.firstName || 'agent pic'} />
          <h5 className='text-xs font-medium leading-3 text-black-variant'>
            @{tour.agent?.firstName} <span className='opacity-70'>from</span> {tour.agent?.locationCity},{tour.agent?.locationCountry}
          </h5>
        </div>
        ...
        <div className="tour-details text-black-variant text-xs font-normal leading-4 flex flex-col gap-5">
          ...
          <div className="tour-meta flex flex-col gap-2">
            <div className='tour-timings flex flex-row gap-2 items-center'>
              ...
              <span className='text-black-variant text-xs font-normal leading-3'>{tour.duration.hours} hours, {tour.duration.minutes} minutes</span>
            </div>

            <div className='tour-viewers flex flex-row gap-2 items-center'>
              <PersonIcon fontSize='small'  sx={{ color: '#1D1D1F' }} />
              <span className='text-black-variant text-xs font-normal leading-3'>{tour.viewers} viewers</span>
            </div>
          </div>
        </div>
